...
object PrettyPrinter {
  ...
  def print(p: Program): String =
    printPaths(p.references) + printItems(p.constructions)
  ...
  def printPaths(ps: List[Path]): String = (ps map { print } mkString n) + n
  def print(p: Path): String = "include" + s + p.path
  def printItems(is: List[Item]): String = (is map { print } mkString n) + n
  ...
  def print(p: Pattern): String =
    p match {
      ...
      case Tuple(contents)            => "(" + (contents map { print } mkString c) + ")"
    }
  def print(e: Expr): String =
    e match {
      case FnApp(fn, args) =>
        print(fn) + "(" + (args map { print } mkString c) + ")"
      case Exactly(id)             => print(id)
      case SetLit(items)           => "{" + (items map { print } mkString c) + "}"
