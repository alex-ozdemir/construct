...
abstract class ConstructError(val msg: String, val position: Option[Position])
  ...
  val fullMsg: String = {
    position map {pos =>
      ...
      val src = pos.longString.split("\n") zip List(lineStr + "| ", (" " * lineStr.length) + "| ") map {
        case (r, l) => l + r
      } mkString "\n"
      s"Error: $msg\n$src"
...
object ConstructError {
  ...
  case class UnknownIdentifierOfType(id: Identifier, ty: String)
    extends ConstructError(s"Unknown identifier `${id.name}` of type `$ty`", Some(id.pos))
  case class TypeError(v: Value, expected: String)
    extends ConstructError(s"<${v.pretty}> was expected to be a `$expected`!", None)
  case class ParameterTypeError(param: Parameter, v: Value)
    extends ConstructError(
      s"The formal parameter `${param.name.name}` is supposed to have type `${param.ty}` but the actual parameter was <${v.pretty}>!", None)
  case class UsedIdentifier(id: Identifier)
    extends ConstructError(s"The identifier `${id.name}` has already been used", Some(id.pos))
  case class SelfIntersection(fn: FnApp, v: Value)
    extends ConstructError(s"The function application `${PrettyPrinter.print(fn)}` is trying to intersect <${v.pretty}> with itself", Some(fn.pos))
  ...
  case class ImplicitGiven(param: Parameter)
    extends ConstructError(
      s"Implicit givens must be points, but `${PrettyPrinter.print(param.name)}` is a `${param.ty}`", None)
  case class Rebind(pattern: Pattern, ident: Identifier)
    extends ConstructError(s"The pattern `${PrettyPrinter.print(pattern)}` rebinds the variable `${ident.name}`", Some(pattern.pos))
  case class Arity(c: Construction, vars: List[Value])
    extends ConstructError(
      s"The construction `${c.name.name}` expects ${c.parameters.length} parameters but got ${vars.length}", None)
  case class BuiltinArity(b: Builtins.Function, vars: List[Value])
    extends ConstructError(
      s"The builtin `${b.name}` expects ${b.arity} parameters but got ${vars.length}", None)
  case class BuiltinBindError(b: Builtins.Type, pats: List[Pattern])
    extends ConstructError(
      s"The builtin `${b.name}` can be broken into ${b.arity} points, but you tried to break it into ${pats.length} items", None)
  abstract class BindError(value_ty: String, v: Value, p: Pattern, reason: String) extends ConstructError(
    s"Cannot bind the $value_ty <${v.pretty}> to the pattern `${PrettyPrinter.print(p)}. $reason", Some(p.pos)
  )
  ...
  case class WebInclude(file: String)
    extends ConstructError(s"I cannot include `$file`, as web construct does not support includes", None)
}